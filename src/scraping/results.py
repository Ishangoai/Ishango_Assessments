...
def extract_results(
    ...
    results = T.retrieve_and_union_results(
        assessments=D.Assessments.ghana_2022_october_assessments,
        session=session,
    )
    ...
    if pre_process:
        ...
        results = T.pre_process_results(
            dataframe=results,
            col_types=D.PandasSchemas.ghana_2022_schema.value,
        )
    ...
    if save_to_db:
        ...
        db = T.DataBaseInteraction(
            dataframe=results,
            table_name=D.DatabaseTables.TABLE_ghana_2022.value,
        )
